#include "types.h"
#include "param.h"
#include "memlayout.h"
#include "spinlock.h"
#include "riscv.h"
#include "defs.h"

// Buddy allocator
/**
 * 一对兄弟块共用一个bit来记录它们的alloc情况
 * B1_is_free XOR B2_is_free
*/

static int nsizes;     // the number of entries in bd_sizes array

#define LEAF_SIZE     16                         // The smallest block size
#define MAXSIZE       (nsizes-1)                 // Largest index in bd_sizes array
#define BLK_SIZE(k)   ((1L << (k)) * LEAF_SIZE)  // Size of block at size k
#define HEAP_SIZE     BLK_SIZE(MAXSIZE) 
#define NBLK(k)       (1 << (MAXSIZE-k))         // Number of block at size k
#define ROUNDUP(n,sz) (((((n)-1)/(sz))+1)*(sz))  // Round up to the next multiple of sz

typedef struct list Bd_list;

// The allocator has sz_info for each size k. Each sz_info has a free
// list, an array alloc to keep track which blocks have been
// allocated, and an split array to to keep track which blocks have
// been split.  The arrays are of type char (which is 1 byte), but the
// allocator uses 1 bit per block (thus, one char records the info of
// 8 blocks).
struct sz_info {
  Bd_list free;
  char *alloc;
  char *split;
};
typedef struct sz_info Sz_info;

static Sz_info *bd_sizes; 
static void *bd_base;   // start address of memory managed by the buddy allocator
static struct spinlock lock;

// Return 1 if bit at position index in array is set to 1
// 判断array[index] == 1？是：返回1；不是，返回0
// 保留，因为split
int bit_isset(char *array, int index) {
  char b = array[index/8];
  char m = (1 << (index % 8));
  return (b & m) == m;
}

// Set bit at position index in array to 1
void bit_set(char *array, int index) {
  char b = array[index/8];
  char m = (1 << (index % 8));
  array[index/8] = (b | m);
}

// Clear bit at position index in array
void bit_clear(char *array, int index) {
  char b = array[index/8];
  char m = (1 << (index % 8));
  array[index/8] = (b & ~m);
}

// --------------------------------------------------------------------------wsl
// 异或，为了给alloc赋值
// 判断array[index] == 1？是：返回1；不是，返回0
void bit_toggle(char *array,int index){
  index/=2;   //因为一对buddy用1个bit所以要/2
  char b = array[index/8];
  char m = (1 << (index % 8));
  array[index/8] = (b ^ m);
}

//如果数组中位置索引处的位为1，则返回1
//表示一个空闲，另一个被分配
// 等价于bit_sset
// --------------------------------------------------------------------------wsl
int bit_get(char *array,int index){
  index /= 2;
  char b = array[index/8];  //因为index的单位是Byte 像实验五位图
  char m = (1 << (index % 8));
  return (b & m) == m;
}

// Print a bit vector as a list of ranges of 1 bits
void
bd_print_vector(char *vector, int len) {
  int last, lb;
  
  last = 1;
  lb = 0;
  for (int b = 0; b < len; b++) {
    if (last == bit_isset(vector, b))
      continue;
    if(last == 1)
      printf(" [%d, %d)", lb, b);
    lb = b;
    last = bit_isset(vector, b);
  }
  if(lb == 0 || last == 1) {
    printf(" [%d, %d)", lb, len);
  }
  printf("\n");
}

// Print buddy's data structures
void
bd_print() {
  for (int k = 0; k < nsizes; k++) {
    printf("size %d (blksz %d nblk %d): free list: ", k, BLK_SIZE(k), NBLK(k));
    lst_print(&bd_sizes[k].free);
    printf("  alloc:");
    bd_print_vector(bd_sizes[k].alloc, NBLK(k));
    if(k > 0) {
      printf("  split:");
      bd_print_vector(bd_sizes[k].split, NBLK(k));
    }
  }
}

// What is the first k such that 2^k >= n?
int
firstk(uint64 n) {
  int k = 0;
  uint64 size = LEAF_SIZE;

  while (size < n) {
    k++;
    size *= 2;
  }
  return k;
}

// Compute the block index for address p at size k
int
blk_index(int k, char *p) {
  int n = p - (char *) bd_base;
  return n / BLK_SIZE(k);
}

// Convert a block index at size k back into an address
void *addr(int k, int bi) {
  int n = bi * BLK_SIZE(k);
  return (char *) bd_base + n;
}

// allocate nbytes, but malloc won't return anything smaller than LEAF_SIZE
void *
bd_malloc(uint64 nbytes)
{
  int fk, k;

  acquire(&lock);

  // 找到一个空闲块>=nbytes，从最小的k开始
  fk = firstk(nbytes);
  for (k = fk; k < nsizes; k++) {
    if(!lst_empty(&bd_sizes[k].free))
      break;
  }
  if(k >= nsizes) { //没有空闲块
    release(&lock);
    return 0;
  }

  // 找到一个块；弹出它并可能拆分它。
  char *p = lst_pop(&bd_sizes[k].free);
//---------------------------------------------------------------------------------wsl
  // bit_set(bd_sizes[k].alloc, blk_index(k, p));
  bit_toggle(bd_sizes[k].alloc, blk_index(k, p)); //用1位bit给兄弟block的alloc赋值
  for(; k > fk; k--) {
    // 将一个块拆分为k大小，并将其中一半分配到k-1大小
    // 把他放在k-1的空闲链表上
    char *q = p + BLK_SIZE(k-1);   // p的兄弟
    bit_set(bd_sizes[k].split, blk_index(k, p));
//---------------------------------------------------------------------------------wsl
    // bit_set(bd_sizes[k-1].alloc, blk_index(k-1, p));
    bit_toggle(bd_sizes[k-1].alloc, blk_index(k-1, p)); //用1位bit给兄弟block的alloc赋值
    lst_push(&bd_sizes[k-1].free, q);
  }
  release(&lock);

  return p;
}

// Find the size of the block that p points to.
int
size(char *p) {
  for (int k = 0; k < nsizes; k++) {
    if(bit_isset(bd_sizes[k+1].split, blk_index(k+1, p))) {
      return k;
    }
  }
  return 0;
}

// Free memory pointed to by p, which was earlier allocated using
// bd_malloc.
void
bd_free(void *p) {
  void *q;
  int k;

  acquire(&lock);
  for (k = size(p); k < MAXSIZE; k++) {
    int bi = blk_index(k, p);
    int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
    // bit_clear(bd_sizes[k].alloc, bi);  // free p at size k
//-----------------------------------------------------------------------------------wsl    
    bit_toggle(bd_sizes[k].alloc, bi);  // free p at size k
    // if (bit_isset(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
    if (bit_get(bd_sizes[k].alloc, bi)) {  // is buddy allocated?
      break;   // break out of loop               // as bi is already free,so if xor bit is 1,then buddy is allocated
    }
    // budy is free; merge with buddy
    q = addr(k, buddy);
    lst_remove(q);    // remove buddy from free list
    if(buddy % 2 == 0) {
      p = q;
    }
    // at size k+1, mark that the merged buddy pair isn't split
    // anymore
    bit_clear(bd_sizes[k+1].split, blk_index(k+1, p));
  }
  lst_push(&bd_sizes[k].free, p);
  release(&lock);
}

// Compute the first block at size k that doesn't contain p
int
blk_index_next(int k, char *p) {
  int n = (p - (char *) bd_base) / BLK_SIZE(k);
  if((p - (char*) bd_base) % BLK_SIZE(k) != 0)
      n++;
  return n ;
}

int
log2(uint64 n) {
  int k = 0;
  while (n > 1) {
    k++;
    n = n >> 1;
  }
  return k;
}

// Mark memory from [start, stop), starting at size 0, as allocated. 
void
bd_mark(void *start, void *stop)
{
  int bi, bj;

  if (((uint64) start % LEAF_SIZE != 0) || ((uint64) stop % LEAF_SIZE != 0))
    panic("bd_mark");

  for (int k = 0; k < nsizes; k++) {
    bi = blk_index(k, start);
    bj = blk_index_next(k, stop);
    for(; bi < bj; bi++) {
      if(k > 0) {
        // if a block is allocated at size k, mark it as split too.
        bit_set(bd_sizes[k].split, bi);
      }
      // bit_set(bd_sizes[k].alloc, bi);
//----------------------------------------------------------------------------------------wsl      
      bit_toggle(bd_sizes[k].alloc,bi);
    }
  }
} 

//----------------------------------------------------------------------------------------wsl      
// 如果addr在left和right指向的地址之间，返回1
int addr_in_left_right(void *addr,void *left,void *right,int size){
  return (addr>=left)&&((addr+size)<right);
}

// If a block is marked as allocated and the buddy is free, put the
// buddy on the free list at size k.
//----------------------------------------------------------------------------------------wsl 
// 重点-初始化各层空链表
int
bd_initfree_pair(int k, int bi,void *left,void *right) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  // if(bit_isset(bd_sizes[k].alloc, bi) !=  bit_isset(bd_sizes[k].alloc, buddy)) { 
  if(bit_get(bd_sizes[k].alloc,bi)){  //返回的是1，表示bi和buddy的状态不一样，只有1个是free
    // one of the pair is free
    free = BLK_SIZE(k);
    // printf("bi (%p,%p,in range:%d)\tbuddy(%p,%p,in range:%d)\n",addr(k,bi),addr(k,bi)+free,addr_in_left_right(addr(k,bi),left,right,free),addr(k,buddy),addr(k,buddy)+free,addr_in_left_right(addr(k,buddy),left,right,free));
    if(addr_in_left_right(addr(k,bi),left,right,free)){
      lst_push(&bd_sizes[k].free, addr(k, bi)); //把自己加入空闲链表
    }else{
      lst_push(&bd_sizes[k].free, addr(k, buddy));  //把buddy加入空闲链表
    }
    // if(bit_isset(bd_sizes[k].alloc, bi))
    //   lst_push(&bd_sizes[k].free, addr(k, buddy));   // put buddy on free list
    // else
    //   lst_push(&bd_sizes[k].free, addr(k, bi));      // put bi on free list
  }
  return free;
}
  

// 初始化每个大小k的空闲列表。对于每个大小k，
// 在此处只有两对可能会有一对buddy在freelist上：
// 左和右。
//--------------------------------------------------------------------------------wsl
int
bd_initfree(void *bd_left, void *bd_right) {
  int free = 0;

  for (int k = 0; k < MAXSIZE; k++) {   // skip max size
    int left = blk_index_next(k, bd_left);
    int right = blk_index(k, bd_right);
    //free += bd_initfree_pair(k, left);    
    free += bd_initfree_pair(k, left, bd_left,bd_right);
    if(right <= left) 
      continue;
    //free += bd_initfree_pair(k, right);
    free += bd_initfree_pair(k, right, bd_left,bd_right);
  }
  return free;
}

// Mark the range [bd_base,p) as allocated
int
bd_mark_data_structures(char *p) {
  int meta = p - (char*)bd_base;
  printf("bd: %d meta bytes for managing %d bytes of memory\n", meta, BLK_SIZE(MAXSIZE));
  bd_mark(bd_base, p);
  return meta;
}

// Mark the range [end, HEAPSIZE) as allocated
int
bd_mark_unavailable(void *end, void *left) {
  int unavailable = BLK_SIZE(MAXSIZE)-(end-bd_base);
  if(unavailable > 0)
    unavailable = ROUNDUP(unavailable, LEAF_SIZE);
  printf("bd: 0x%x bytes unavailable\n", unavailable);

  void *bd_end = bd_base+BLK_SIZE(MAXSIZE)-unavailable;
  bd_mark(bd_end, bd_base+BLK_SIZE(MAXSIZE));
  return unavailable;
}

// Initialize the buddy allocator: it manages memory from [base, end).
void
bd_init(void *base, void *end) {
  char *p = (char *) ROUNDUP((uint64)base, LEAF_SIZE);
  int sz;

  initlock(&lock, "buddy");
  bd_base = (void *) p;

  // 计算我们需要管理的大小 [base, end)
  nsizes = log2(((char *)end-p)/LEAF_SIZE) + 1;
  if((char*)end-p > BLK_SIZE(MAXSIZE)) {
    nsizes++;  // round up to the next power of 2
  }

  printf("bd: memory sz is %d bytes; allocate an size array of length %d\n",
         (char*) end - p, nsizes);

  // allocate bd_sizes array
  bd_sizes = (Sz_info *) p;
  p += sizeof(Sz_info) * nsizes;
  memset(bd_sizes, 0, sizeof(Sz_info) * nsizes);

// 初始化空闲列表并为每个大小k分配alloc数组
  for (int k = 0; k < nsizes; k++) {
    lst_init(&bd_sizes[k].free);
//-----------------------------------------------------------------------------------wsl    
    sz = sizeof(char)* ROUNDUP(NBLK(k), 16)/16;
    bd_sizes[k].alloc = p;
    memset(bd_sizes[k].alloc, 0, sz);
    p += sz;
  }

// 为每个大小k分配分割数组，除了k=0，
// 因为不会拆分大小为k=0（最小大小）的块。
  for (int k = 1; k < nsizes; k++) {
    sz = sizeof(char)* (ROUNDUP(NBLK(k), 8))/8;
    bd_sizes[k].split = p;
    memset(bd_sizes[k].split, 0, sz);
    p += sz;
  }
  p = (char *) ROUNDUP((uint64) p, LEAF_SIZE);

// 完成分配；将内存范围[base，p]标记为已分配，
  int meta = bd_mark_data_structures(p);
  
// 将不可用内存范围[结束，堆大小]标记为已分配，
// 这样buddy就不会出问题
  int unavailable = bd_mark_unavailable(end, p);
  void *bd_end = bd_base+BLK_SIZE(MAXSIZE)-unavailable;
  
  // 为每个大小k初始化空闲列表
  int free = bd_initfree(p, bd_end);

  // 检查空闲链表的大小是否符合我们的期望
  if(free != BLK_SIZE(MAXSIZE)-meta-unavailable) {
    printf("free %d %d\n", free, BLK_SIZE(MAXSIZE)-meta-unavailable);
    panic("bd_init: free mem");
  }
}
